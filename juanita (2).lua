...
ui.sections = {
	...
	arrow_esp = ui.subtabs.visuals_esp:Section({Name = "Arrow ESP", Side = "Right"}),
...
-- ---- ESP Preview Panel Creation ----
do
	local PreviewGui = NewLib.Holder
	local preview_frame = NewLib:Create("Frame", {
		Name = "\0",
		Parent = PreviewGui,
		Position = UDim2.new(0.75, 0, 0.5, 0),
		AnchorPoint = Vector2.new(0.5, 0.5),
		Size = UDim2.new(0, 220, 0, 260),
		BackgroundColor3 = NewLib.Theme["Background"],
		BorderSizePixel = 0,
		ClipsDescendants = true,
		Visible = false
	}):AddToTheme({BackgroundColor3 = 'Background'})

	NewLib:Create("UIStroke", {
		Parent = preview_frame.Instance,
		ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
		LineJoinMode = Enum.LineJoinMode.Miter,
		Color = NewLib.Theme["Border"],
		BorderOffset = UDim.new(0, 1)
	}):AddToTheme({Color = 'Border'})
	...
	NewLib:Create("UIStroke", {
		...
		ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
		LineJoinMode = Enum.LineJoinMode.Miter,
		Color = NewLib.Theme["Outline"]
	}):AddToTheme({Color = 'Outline'})

	-- Title bar
	local preview_title = NewLib:Create("TextLabel", {
		Name = "\0",
		Parent = preview_frame.Instance,
		Text = "ESP Preview",
		TextColor3 = NewLib.Theme["Accent"],
		TextSize = NewLib.FontSize,
		FontFace = NewLib.Font,
		TextXAlignment = Enum.TextXAlignment.Left,
		BackgroundTransparency = 1,
		Position = UDim2.new(0, 8, 0, 6),
	...
	-- Tabs (Enemies / Allies / Bots / Vehicles)
	local preview_tabs = NewLib:Create("Frame", {
		Name = "\0",
		Parent = preview_frame.Instance,
		BackgroundTransparency = 1,
	...
	local tab_names = {"Enemies", "Allies", "Bots", "Vehicles"}
	local preview_tab_data = {}
	local active_preview_tab = "Enemies"
	for i, tname in ipairs(tab_names) do
		local tab_btn = NewLib:Create("TextButton", {
			Name = "\0",
			Parent = preview_tabs.Instance,
			Text = tname,
			TextColor3 = NewLib.Theme["Inactive Text"],
			TextSize = 9,
			FontFace = NewLib.Font,
			AutoButtonColor = false,
		...
		NewLib:Create("UIStroke", {
			Parent = tab_btn.Instance,
			ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
			Color = NewLib.Theme["Outline"]
		}):AddToTheme({Color = 'Outline'})

		preview_tab_data[tname] = tab_btn
		tab_btn:Connect("MouseButton1Down", function()
			active_preview_tab = tname
			for _, t in pairs(preview_tab_data) do
				t:Tween({BackgroundColor3 = Color3.fromRGB(20,20,20)})
				t.Instance.TextColor3 = NewLib.Theme["Inactive Text"]
			end
			tab_btn:Tween({BackgroundColor3 = NewLib.Theme["Element 2"]})
			tab_btn.Instance.TextColor3 = NewLib.Theme["Accent"]
			update_preview_character()
		end)
	end

	-- Active tab highlight
	preview_tab_data["Enemies"]:Tween({BackgroundColor3 = NewLib.Theme["Element 2"]})
	preview_tab_data["Enemies"].Instance.TextColor3 = NewLib.Theme["Accent"]

	-- Preview area
	local preview_area = NewLib:Create("Frame", {
		Name = "\0",
		Parent = preview_frame.Instance,
	...
	NewLib:Create("UIStroke", {
		Parent = preview_area.Instance,
		ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
		Color = NewLib.Theme["Outline"]
	}):AddToTheme({Color = 'Outline'})

	-- Roblox character silhouette (simple blocks)
	local char_frame = NewLib:Create("Frame", {
		Name = "\0",
		Parent = preview_area.Instance,
		BackgroundColor3 = Color3.fromRGB(100, 100, 100),
		Position = UDim2.new(0.5, -20, 0.1, 10),
		Size = UDim2.new(0, 40, 0, 44),
		BorderSizePixel = 0
	})
	-- Head
	local char_head = NewLib:Create("Frame", {
		Name = "\0",
		Parent = preview_area.Instance,
		BackgroundColor3 = Color3.fromRGB(100, 100, 100),
		Position = UDim2.new(0.5, -10, 0.1, 0),
	...
	-- ESP element labels at bottom of preview
	local esp_labels = NewLib:Create("Frame", {
		Name = "\0",
		Parent = preview_area.Instance,
		BackgroundTransparency = 1,
	...
	NewLib:Create("UIListLayout", {
		Parent = esp_labels.Instance,
		FillDirection = Enum.FillDirection.Horizontal,
	...
	local preview_elem_name = NewLib:Create("TextLabel", {
		Name = "\0",
		Parent = esp_labels.Instance,
		Text = "Name",
		TextColor3 = NewLib.Theme["Text"],
		TextSize = 8,
		FontFace = NewLib.Font,
		BackgroundTransparency = 1,
		BorderSizePixel = 0,
	...
	local preview_elem_dist = NewLib:Create("TextLabel", {
		Name = "\0",
		Parent = esp_labels.Instance,
		Text = "Distance",
		TextColor3 = NewLib.Theme["Text"],
	...
	local preview_elem_health = NewLib:Create("TextLabel", {
		Name = "\0",
		Parent = esp_labels.Instance,
		Text = "Health",
		TextColor3 = NewLib.Theme["Text"],
	...
	local preview_elem_box = NewLib:Create("TextLabel", {
		Name = "\0",
		Parent = esp_labels.Instance,
		Text = "Box",
		TextColor3 = NewLib.Theme["Text"],
	...
	-- Bottom info
	local preview_info = NewLib:Create("Frame", {
		Name = "\0",
		Parent = preview_frame.Instance,
		BackgroundTransparency = 1,
		Position = UDim2.new(0, 8, 0, 194),
		Size = UDim2.new(1, -16, 0, 30),
		BorderSizePixel = 0
	})

	local preview_enabled_toggle = NewLib:Create("TextLabel", {
		Name = "\0",
		Parent = preview_info.Instance,
		Text = "☐ ESP Enabled",
		TextColor3 = NewLib.Theme["Text"],
		TextSize = 10,
		FontFace = NewLib.Font,
	...
	local preview_distance = NewLib:Create("TextLabel", {
		Name = "\0",
		Parent = preview_info.Instance,
		Text = "• 0m",
		TextColor3 = NewLib.Theme["Inactive Text"],
	...
	local preview_slider_label = NewLib:Create("TextLabel", {
		Name = "\0",
		Parent = preview_info.Instance,
	...
	local preview_render_dist = NewLib:Create("Slider", {
		Parent = preview_info.Instance,
		BackgroundColor3 = NewLib.Theme["Element"],
	...
	NewLib:Create("Frame", {
		Name = "\0",
		Parent = preview_render_dist.Instance,
		BackgroundColor3 = NewLib.Theme["Accent"],
		Size = UDim2.new(0.5, 0, 1, 0),
		BorderSizePixel = 0
	}):AddToTheme({BackgroundColor3 = 'Accent'})

	-- Make draggable
	preview_frame.Instance.Draggable = true
	preview_frame.Instance.Active = true

	esp_preview_panel = preview_frame

	function update_preview_character()
		if not esp_preview_panel or not esp_preview_panel.Instance.Visible then return end
		local is_enemy = active_preview_tab == "Enemies"
		local is_ally = active_preview_tab == "Allies"

		-- Update checkbox
		preview_enabled_toggle.Instance.Text = (is_enemy and "☑" or "☐") .. " ESP Enabled"

		-- Update element visibility
		preview_elem_name.Instance.Visible = enemy_sets.name
		preview_elem_dist.Instance.Visible = enemy_sets.distance
		preview_elem_health.Instance.Visible = enemy_sets.health_bar
		preview_elem_box.Instance.Visible = enemy_sets.box

		-- Update distance display
		local dist = enemy_main_sets.max_distance
		preview_distance.Instance.Text = "• " .. dist .. "m"

		-- Character color
		local char_color = is_enemy and Color3.fromRGB(200, 80, 80) or (is_ally and Color3.fromRGB(80, 120, 200) or Color3.fromRGB(100, 100, 100))
		char_frame:Tween({BackgroundColor3 = char_color})
		char_head:Tween({BackgroundColor3 = char_color})
	end

	-- Connect ESP settings changes to preview updates
	local preview_update_conn = NewLib:Connect(RunService.Heartbeat, function()
		if esp_preview_panel and esp_preview_panel.Instance.Visible then
			update_preview_character()
		end
	end)
end
...
-- ---- Arrow ESP ----
local arrow_enabled = false
local arrow_distance = 100
local arrow_elements = { name = false, distance = false, health = false, arrow = true }

-- ---- ESP Preview Panel ----
local esp_preview_enabled = false
local esp_preview_panel = nil
do
	...
	-- ---- Arrow ESP Settings ----
	do
		local arrowsec = ui.sections.arrow_esp
		arrowsec:Toggle({Name = "Enabled", Value = false, Flag = "arrow_enabled", Callback = function(bool)
			arrow_enabled = bool
		end})
		arrowsec:Slider({Name = "Max Distance", Min = 10, Max = 2000, Float = 50, Value = 100, Flag = "arrow_distance", Callback = function(int)
			arrow_distance = int
		end})
		arrowsec:Toggle({Name = "Show Arrow", Value = true, Flag = "arrow_show_arrow", Callback = function(bool)
			arrow_elements.arrow = bool
		end})
		arrowsec:Toggle({Name = "Show Name", Value = true, Flag = "arrow_show_name", Callback = function(bool)
			arrow_elements.name = bool
		end})
		arrowsec:Toggle({Name = "Show Distance", Value = true, Flag = "arrow_show_distance", Callback = function(bool)
			arrow_elements.distance = bool
		end})
		arrowsec:Toggle({Name = "Show Health", Value = false, Flag = "arrow_show_health", Callback = function(bool)
			arrow_elements.health = bool
		end})
	end

	-- ---- ESP Preview Panel ----
	do
		local prevsec = ui.sections.esp_preview
		prevsec:Toggle({Name = "Enabled", Value = false, Flag = "esp_preview_enabled", Callback = function(bool)
			esp_preview_enabled = bool
			if esp_preview_panel then
				esp_preview_panel.Instance.Visible = bool
			end
		end})
	end
...
cheat.utility.new_renderstepped(function()
	...
	if not cam or not arrow_enabled then return end
	local cam_pos = cam.CFrame.Position
	local arrow_elements_local = arrow_elements
	...
	for _, player in ipairs(Players:GetPlayers()) do
		...
		local dist = (cam_pos - hrp.Position).Magnitude
		if dist > arrow_distance then continue end

		-- Only render if ESP is not already showing this player (avoid double-rendering)
		local esp_obj = cheat.EspLibrary._loaded_esp and cheat.EspLibrary._loaded_esp[player]
		local esp_visible = enemy_sets.enabled
			and (esp_obj and esp_obj.setvis_cache)
		if esp_visible then continue end

		-- Get on-screen position
		local screen_pos, on_screen = _WorldToViewportPoint(cam, hrp.Position)
		if on_screen then continue end -- already on screen, don't need arrow

		-- Arrow position at screen edge
		local vp_size = cam.ViewportSize
		local dir = _Vector2new(screen_pos.X - vp_size.X/2, screen_pos.Y - vp_size.Y/2).Unit
		local arrow_margin = 50
		local arrow_pos = _Vector2new(vp_size.X/2, vp_size.Y/2) + dir * (math.min(vp_size.X, vp_size.Y)/2 - arrow_margin)
		local angle = math.atan2(dir.Y, dir.X) * (180 / math.pi)

		-- Draw arrow using Drawing API
		local arrow_id = player.UserId

		-- Arrow indicator (triangle)
		local arrow_obj = arrow_drawings[arrow_id]
		if not arrow_obj then
			arrow_obj = {
				arrow = cheat.utility.new_drawing("Triangle", {}),
				name = cheat.utility.new_drawing("Text", {}),
				dist = cheat.utility.new_drawing("Text", {}),
				health = cheat.utility.new_drawing("Text", {}),
			}
			arrow_drawings[arrow_id] = arrow_obj
		end

		-- Arrow triangle
		if arrow_elements_local.arrow then
			local arrow_color = player.Team == LocalPlayer.Team and Color3.new(0, 0.5, 1) or Color3.new(1, 0, 0)
			local s = 12
			arrow_obj.arrow.Visible = true
			arrow_obj.arrow.Thickness = 2
			arrow_obj.arrow.Filled = true
			arrow_obj.arrow.Color = arrow_color
			arrow_obj.arrow.PointA = arrow_pos + _Vector2new(-s, -s)
			arrow_obj.arrow.PointB = arrow_pos + _Vector2new(s, -s)
			arrow_obj.arrow.PointC = arrow_pos + _Vector2new(0, s)
			arrow_obj.arrow.Rotation = angle
		else
			arrow_obj.arrow.Visible = false
		end

		-- Name label
		if arrow_elements_local.name then
			local name_color = player.Team == LocalPlayer.Team and Color3.new(0.5, 0.7, 1) or Color3.new(1, 0.5, 0.5)
			arrow_obj.name.Visible = true
			arrow_obj.name.Text = player.Name
			arrow_obj.name.Size = 12
			arrow_obj.name.Outline = true
			arrow_obj.name.OutlineColor = Color3.new(0, 0, 0)
			arrow_obj.name.Color = name_color
			arrow_obj.name.Position = arrow_pos + _Vector2new(0, 16)
			arrow_obj.name.Center = true
		else
			arrow_obj.name.Visible = false
		end

		-- Distance label
		if arrow_elements_local.distance then
			arrow_obj.dist.Visible = true
			arrow_obj.dist.Text = math.floor(dist) .. "m"
			arrow_obj.dist.Size = 11
			arrow_obj.dist.Outline = true
			arrow_obj.dist.OutlineColor = Color3.new(0, 0, 0)
			arrow_obj.dist.Color = Color3.new(0.8, 0.8, 0.8)
			arrow_obj.dist.Position = arrow_pos + _Vector2new(0, 30)
			arrow_obj.dist.Center = true
		else
			arrow_obj.dist.Visible = false
		end

		-- Health label
		if arrow_elements_local.health then
			local humanoid = _FindFirstChildOfClass(character, "Humanoid")
			local hp = humanoid and humanoid.Health or 0
			arrow_obj.health.Visible = true
			arrow_obj.health.Text = math.floor(hp)
			arrow_obj.health.Size = 10
			arrow_obj.health.Outline = true
			arrow_obj.health.OutlineColor = Color3.new(0, 0, 0)
			arrow_obj.health.Color = hp > 60 and Color3.new(0, 1, 0) or (hp > 30 and Color3.new(1, 1, 0) or Color3.new(1, 0, 0))
			arrow_obj.health.Position = arrow_pos + _Vector2new(0, 44)
			arrow_obj.health.Center = true
		else
			arrow_obj.health.Visible = false
		end
	end
end)
